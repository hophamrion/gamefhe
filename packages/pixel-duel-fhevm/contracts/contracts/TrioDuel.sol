 ...
 *
 * IMPORTANT: Depending on the version of @fhevm/solidity you install, some API
 * names may differ (e.g., `externalEuint8`, `FHE.fromExternal`, `FHE.requestDecryption`,
 * `SepoliaConfig` import path). Adjust accordingly.
 */
import { FHE, euint8, euint16, ebool } from "@fhevm/solidity/lib/FHE.sol";
// If targeting Sepolia coprocessor / decryption oracle, include the proper config:
...
contract TrioDuel is SepoliaConfig {
    ...
    // ===== FHE helpers =====
    function beats(euint8 a, euint8 b) internal pure returns (ebool) {
    ...
    function applyDamage(euint16 hp, ebool advSkill, ebool advElem) internal pure returns (euint16) {
    ...
    // NOTE: replace `externalEuint8` with the exact external input type of your SDK version
